...
export function GearCard({ gear, onOpenStore, onAddToCart }: GearCardProps) {
  return (
    <motion.div whileHover={{ y: -5 }} transition={{ type: 'spring', stiffness: 300 }}>
      <Card className="group flex h-full flex-col overflow-hidden rounded-xl border-[var(--tone-border)] bg-[var(--tone-surface-soft)] shadow-sm sm:rounded-2xl">
        ...
        <div className="flex flex-1 flex-col p-3 sm:p-5">
          <h3 className="mb-1 line-clamp-1 text-sm font-bold text-[var(--tone-text)] sm:text-lg">{gear.name}</h3>
          <p className="mb-2 line-clamp-2 text-[11px] text-muted-foreground sm:text-sm">{gear.description || 'No description provided.'}</p>
          <p className="mb-3 line-clamp-1 text-[10px] text-muted-foreground sm:mb-4 sm:text-xs">
          ...
          <div className="mb-3 flex items-center gap-2 sm:mb-4 sm:gap-1">
            ...
            <button className="line-clamp-1 text-left text-xs font-semibold hover:underline sm:text-sm" onClick={() => onOpenStore(gear.store.id)}>
              {gear.store.name}
            </button>
          </div>
          <div className="mt-auto flex items-center justify-between gap-3">
            <div>
              <p className="font-bold text-[var(--tone-text)] sm:text-base)">{formatPHP(Number(gear.daily_price || 0))}</p>
              <p className="text-[10px] text-muted-foreground sm:text-xs">per day</p>
            </div>
            <Button className="h-8 gap-2 bg-[var(--tone-accent)] text-xs text-[var(--tone-accent-text)] hover:opacity-90 sm:h-9 sm:text-sm" onClick={() => onAddToCart(gear)}>
