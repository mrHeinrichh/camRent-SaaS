...
export function GearCard({ gear, onOpenStore, onAddToCart }: GearCardProps) {
  return (
    <motion.div whileHover={{ y: -5 }} transition={{ type: 'spring', stiffness: 300 }}>
      <Card className="group overflow-hidden rounded-2xl border-[var(--tone-border)] bg-[var(--tone-surface-soft)] shadow-sm">
        ...
        <div className="p-5">
          <h3 className="mb-1 text-lg font-bold text-[var(--tone-text)]">{gear.name}</h3>
          ...
          <div className="flex items-center justify-between gap-3">
            <div>
              <p className="text-lg font-bold text-[var(--tone-text)]">{formatPHP(Number(gear.daily_price || 0))}</p>
              <p className="text-xs text-muted-foreground">per day</p>
            </div>
            <Button className="gap-2 bg-[var(--tone-accent)] text-[var(--tone-accent-text)] hover:opacity-90" onClick={() => onAddToCart(gear)}>
