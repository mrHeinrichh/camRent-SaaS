...
export function GearCard({ gear, onOpenStore, onAddToCart }: GearCardProps) {
  return (
    <motion.div whileHover={{ y: -5 }} transition={{ type: 'spring', stiffness: 300 }}>
      <Card className="group flex h-full flex-col overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
        ...
        <div className="flex flex-1 flex-col p-4 sm:p-5">
          ...
          <div className="mt-auto flex items-center justify-between gap-3">
            ...
            <Button className="h-8 gap-2 rounded-full bg-[var(--tone-accent)] text-xs text-[var(--tone-accent-text)] hover:opacity-90 sm:h-9 sm:text-sm" onClick={() => onAddToCart(gear)}>
