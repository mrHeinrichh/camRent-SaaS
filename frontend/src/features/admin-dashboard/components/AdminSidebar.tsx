...
export function AdminSidebar({ activeTab, onChangeTab, pendingMerchants = 0, nearDueStores = 0, pendingGlobalFraud = 0, feedbackCount = 0 }: AdminSidebarProps) {
  return (
    <aside className="w-full border-b bg-muted/20 p-2 md:w-64 md:border-b-0 md:border-r md:p-6">
      ...
      <Button variant={activeTab === 'stores' ? 'secondary' : 'ghost'} className="i3d-tab shrink-0 justify-start whitespace-nowrap md:w-full" onClick={() => onChangeTab('stores')}>
      ...
      <Button variant={activeTab === 'customers' ? 'secondary' : 'ghost'} className="i3d-tab shrink-0 justify-start whitespace-nowrap md:w-full" onClick={() => onChangeTab('customers')}>
        <Users className="mr-2 h-4 w-4" /> Customers
      </Button>
      <Button variant={activeTab === 'fraud' ? 'secondary' : 'ghost'} className="i3d-tab shrink-0 justify-start whitespace-nowrap md:w-full" onClick={() => onChangeTab('fraud')}>
      ...
      <Button variant={activeTab === 'insights' ? 'secondary' : 'ghost'} className="i3d-tab shrink-0 justify-start whitespace-nowrap md:w-full" onClick={() => onChangeTab('insights')}>
        <LayoutDashboard className="mr-2 h-4 w-4" /> Insights
      </Button>
      <Button variant={activeTab === 'support' ? 'secondary' : 'ghost'} className="i3d-tab shrink-0 justify-start whitespace-nowrap md:w-full" onClick={() => onChangeTab('support')}>
      ...
      <Button variant={activeTab === 'announcements' ? 'secondary' : 'ghost'} className="i3d-tab shrink-0 justify-start whitespace-nowrap md:w-full" onClick={() => onChangeTab('announcements')}>
        <Megaphone className="mr-2 h-4 w-4" /> Announcements
      </Button>
      <Button variant={activeTab === 'donations' ? 'secondary' : 'ghost'} className="i3d-tab shrink-0 justify-start whitespace-nowrap md:w-full" onClick={() => onChangeTab('donations')}>
        <HandHeart className="mr-2 h-4 w-4" /> Donations
      </Button>
      <Button variant={activeTab === 'content' ? 'secondary' : 'ghost'} className="i3d-tab shrink-0 justify-start whitespace-nowrap md:w-full" onClick={() => onChangeTab('content')}>
